...
func (m Message) GenRpcSearchReqMessage(buf *bytes.Buffer) {
	...
	for _, field := range m.Fields {
		...
		// field.Name 结尾包含 Time
		if strings.HasSuffix(field.Name, "Time") {
			filedTag++
			var item1 MessageField
			var item2 MessageField
			copier.Copy(&item1, &field)
			copier.Copy(&item2, &field)
			item1.Name = item1.Name + "Start"
			item1.Comment = item1.Comment + "Start"
			item2.Name = item2.Name + "End"
			item2.Comment = item2.Comment + "End"
			item2.tag = filedTag
			curFields = append(curFields, item1)
			curFields = append(curFields, item2)
			continue
		}
		curFields = append(curFields, field)
	}
